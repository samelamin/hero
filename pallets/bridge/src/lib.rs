...
//! # Bridge pallet
//!
//! This pallet implements a bridge between Chainbridge and Hero.
//! - [`Config`]
//! - [`Call`]
//! - [`Pallet`]
//!
...
//! - ChainBridge-substrate by Centrifuge [`chainbrige` pallet](https://github.com/centrifuge/chainbridge-substrate).
//!
//! ## References
...
use frame_support::{
	...
	traits::{Currency, EnsureOrigin, ExistenceRequirement::{self, AllowDeath}, Get, WithdrawReasons}, PalletId, transactional
};
...
pub mod pallet {
    ...
    + TypeInfo;
		...
		/// Type for setting fee that are charged when transferring native tokens to target chains (in token units).
		#[pallet::constant]
		type NativeTokenTransferFee: Get<u128>;

		/// Type for setting fee that are charged when transferring NFT tokens to target chains (in token units).
		#[pallet::constant]
	...
	// Additional fee charged when transferring native tokens to target chains (in token units).
	#[pallet::storage]
	...
	// Additional fee charged when transferring NFT tokens to target chains (in token units).
	#[pallet::storage]
    ...
    #[transactional]
		pub fn transfer_native(
			...
			// Burn additional fees, from pallet-fees::burn_fee()
