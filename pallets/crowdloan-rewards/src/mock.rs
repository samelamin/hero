...
pub(crate) fn get_ed25519_pairs(num: u32) -> Vec<ed25519::Pair> {
	...
	for i in 0..num {
		pairs.push(ed25519::Pair::from_seed(
			(seed.clone() + i as u128).to_string().as_bytes().try_into().unwrap(),
		))
	}
	pairs
}
...
pub(crate) fn events() -> Vec<super::Event<Test>> {
	System::events()
		.into_iter()
		.map(|r| r.event)
		.filter_map(|e| if let Event::Crowdloan(inner) = e { Some(inner) } else { None })
		.collect::<Vec<_>>()
}
...
pub(crate) fn batch_events() -> Vec<pallet_utility::Event> {
	System::events()
		.into_iter()
		.map(|r| r.event)
		.filter_map(|e| if let Event::Utility(inner) = e { Some(inner) } else { None })
		.collect::<Vec<_>>()
}
