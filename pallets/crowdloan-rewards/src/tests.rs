...
fn geneses() {
	empty().execute_with(|| {
		...
		assert_ok!(Crowdloan::complete_initialization(Origin::root(), init_block + VESTING));
...
//test associate_native_identity()
#[test]
fn proving_assignation_works() {
	...
	payload.append(&mut 3u64.encode()); //native_account
	...
	empty().execute_with(|| {
		...
		assert!(Crowdloan::accounts_payable(&3).is_none());
		assert_eq!(
			Crowdloan::accounts_payable(&1).unwrap().contributed_relay_addresses,
			vec![[1u8; 32]]
		);

		roll_to(4);
		// Signature is wrong, prove fails
		// associate_native_identity(origin, native_account, relay_account, proof)
		assert_noop!(
			Crowdloan::associate_native_identity(
				Origin::signed(4),
				4, //payload has native_account of 3u64 != 4
				pairs[0].public().into(),
				signature.clone()
			),
		...
		assert_noop!(
			Crowdloan::associate_native_identity(
				Origin::signed(4),
				1,                        //already associated with [1u8; 32]
				pairs[0].public().into(), //none => unassociatedContributions
				alread_associated_signature
			),
			Error::<Test>::AlreadyAssociated
		); //pairs[0].sign(&1u64.encode()).into();

		// Signature is right, prove passes
		// associate_native_identity(origin, native_account, relay_account, proof)
		assert_ok!(Crowdloan::associate_native_identity(
			Origin::signed(4),
			3, //this native_account is in the payload as 3u64.encode()
			pairs[0].public().into(),
			signature.clone()
		)); //signature = pairs[0].sign(&payload).into();

		// Signature is right, but relay address is no longer on unassociated
		assert_noop!(
			Crowdloan::associate_native_identity(
				Origin::signed(4),
				3, //already been thru associate_native_identity
				pairs[0].public().into(),
				signature
			),
			Error::<Test>::NoAssociatedClaim
		); //UnassociatedContributions::<T>::get(&relay_account)...

		// now &3 is payable
		assert!(Crowdloan::accounts_payable(&3).is_some());
		assert_eq!(
			Crowdloan::accounts_payable(&3).unwrap().contributed_relay_addresses,
		...
		]; //initialize_reward_vec() made first 3 events
...
fn initializing_multi_relay_to_single_native_address_works() {
	empty().execute_with(|| {
		...
		assert_ok!(Crowdloan::complete_initialization(Origin::root(), init_block + VESTING)); // 2+8 = 10
																					  // 1 is payable
		assert!(Crowdloan::accounts_payable(&1).is_some());
		assert_eq!(
			Crowdloan::accounts_payable(&1).unwrap().contributed_relay_addresses,
		...
		roll_to(4); //2~4 -> 0.25 of period
		assert_ok!(Crowdloan::claim(Origin::signed(1)));
		assert_eq!(Crowdloan::accounts_payable(&1).unwrap().claimed_reward, 400); //see events() below
		assert_noop!(Crowdloan::claim(Origin::signed(3)), Error::<Test>::NoAssociatedClaim);

		let expected = vec![
			crate::Event::InitialPaymentMade(1, 100),
			crate::Event::InitialPaymentMade(1, 100),
			crate::Event::RewardsPaid(1, 200),
		]; //see mock.rs for initializationPayment
		assert_eq!(events(), expected);
	});
}
...
fn paying_works_step_by_step() {
	empty().execute_with(|| {
		...
		// 1 is payable
		assert!(Crowdloan::accounts_payable(&1).is_some());
		roll_to(4); // up to 0.25 of period
		assert_ok!(Crowdloan::claim(Origin::signed(1))); // acct1 c1
		assert_eq!(Crowdloan::accounts_payable(&1).unwrap().claimed_reward, 200);
		assert_noop!(Crowdloan::claim(Origin::signed(3)), Error::<Test>::NoAssociatedClaim);
		roll_to(5);
		assert_ok!(Crowdloan::claim(Origin::signed(1))); // acct1 c2
		assert_eq!(Crowdloan::accounts_payable(&1).unwrap().claimed_reward, 250);
		roll_to(6);
		assert_ok!(Crowdloan::claim(Origin::signed(1))); // acct1 c3
		assert_eq!(Crowdloan::accounts_payable(&1).unwrap().claimed_reward, 300);
		roll_to(7);
		assert_ok!(Crowdloan::claim(Origin::signed(1))); // acct1 c4
		assert_eq!(Crowdloan::accounts_payable(&1).unwrap().claimed_reward, 350);
		roll_to(8);
		assert_ok!(Crowdloan::claim(Origin::signed(1))); // acct1 c5
		assert_eq!(Crowdloan::accounts_payable(&1).unwrap().claimed_reward, 400);
		roll_to(9);
		assert_ok!(Crowdloan::claim(Origin::signed(1))); // acct1 c6
		assert_eq!(Crowdloan::accounts_payable(&1).unwrap().claimed_reward, 450);
		roll_to(10);
		assert_ok!(Crowdloan::claim(Origin::signed(1))); // acct1 c7
		assert_eq!(Crowdloan::accounts_payable(&1).unwrap().claimed_reward, 500);
		roll_to(11);
		assert_noop!(Crowdloan::claim(Origin::signed(1)), Error::<Test>::RewardsAlreadyClaimed); // acct1 c8
...
fn paying_works_after_unclaimed_period() {
	empty().execute_with(|| {
		...
		assert_eq!(Crowdloan::accounts_payable(&1).unwrap().claimed_reward, 500);
		roll_to(330);
		assert_noop!(Crowdloan::claim(Origin::signed(1)), Error::<Test>::RewardsAlreadyClaimed);
...
fn update_reward_address_works() {
	empty().execute_with(|| {
		...
		roll_to(4);
		assert_ok!(Crowdloan::claim(Origin::signed(1)));
		assert_noop!(Crowdloan::claim(Origin::signed(8)), Error::<Test>::NoAssociatedClaim);
...
fn update_address_with_existing_with_multi_address_works() {
	empty().execute_with(|| {
		...
		assert_noop!(Crowdloan::claim(Origin::signed(1)), Error::<Test>::NoAssociatedClaim);
	});
}
...
fn initialize_new_addresses_not_matching_funds() {
	empty().execute_with(|| {
		...
		)); //5 reward_difference !< 4 TotalContributors
		assert_noop!(
	...
	empty().execute_with(|| {
		...
		let expected = vec![
			...
			pallet_utility::Event::BatchInterrupted {
				index: 0,
				error: DispatchError::Module(ModuleError { index: 2, error: 8, message: None }),
			},
		];
...
fn floating_point_arithmetic_works() {
	empty().execute_with(|| {
		...
		assert_eq!(Crowdloan::accounts_payable(&3).unwrap().claimed_reward, 25u128); // 125 * 0.2

		roll_to(4);
		assert_ok!(Crowdloan::claim(Origin::signed(3)));
		assert_eq!(Crowdloan::accounts_payable(&3).unwrap().claimed_reward, 50u128); // 25 initial + 25 per 2 period

		roll_to(5);
		// rewards should be 12.5 per block, but Rust integer "/" division only produces integer quotients. So 12 will be paid.
		assert_ok!(Crowdloan::claim(Origin::signed(3)));
		assert_eq!(Crowdloan::accounts_payable(&3).unwrap().claimed_reward, 62u128);

		roll_to(6);
		// Account 3 should have claimed 25+50 at this block, but
		// only claimed 62. 50+25 initial −62 = 13
		assert_ok!(Crowdloan::claim(Origin::signed(3)));
		assert_eq!(Crowdloan::accounts_payable(&3).unwrap().claimed_reward, 75u128);
...
fn reward_below_vesting_period_works() {
	empty().execute_with(|| {
		...
		assert_eq!(Crowdloan::accounts_payable(&3).unwrap().claimed_reward, 1u128); // 6/5 => 1 no decimal

		// Here we should pay floor(0.625*2)=1
		// Total claimed reward: 1+1 = 2
		roll_to(4);
		assert_ok!(Crowdloan::claim(Origin::signed(3)));
		assert_eq!(Crowdloan::accounts_payable(&3).unwrap().claimed_reward, 2u128); // 6-1 = 5; 5/8 = 0.625 per blocks, so at block 4, 1+1.25 = 2.25 => 2 no decimal

		roll_to(5);
		assert_ok!(Crowdloan::claim(Origin::signed(3)));
		assert_eq!(Crowdloan::accounts_payable(&3).unwrap().claimed_reward, 2u128); //2.25+0.625 = 2.875 => 2

		roll_to(6);
		assert_ok!(Crowdloan::claim(Origin::signed(3)));
		assert_eq!(Crowdloan::accounts_payable(&3).unwrap().claimed_reward, 3u128); // 2.875+0.625 = 3.5 => 3

		roll_to(10);
		assert_ok!(Crowdloan::claim(Origin::signed(3)));
		assert_eq!(Crowdloan::accounts_payable(&3).unwrap().claimed_reward, 6u128); // 3.5+0.625*4 = 6

		roll_to(11);
		assert_noop!(Crowdloan::claim(Origin::signed(3)), Error::<Test>::RewardsAlreadyClaimed);
...
fn test_relay_signatures_can_change_reward_addresses() {
	empty().execute_with(|| {
		...
		for pair in pairs.clone() {
			assert!(reward_info.contributed_relay_addresses.contains(&pair.public().into()))
		}

		// Threshold is set to 50%(set in mock.rs: RewardAddressRelayVoteThreshold), so we need at least 3 votes to pass
